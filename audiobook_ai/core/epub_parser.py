...
class EPUBParser:
    ...
    def _split_into_sentences(self, text: str) -> str:
        """Split text into sentences, handling French dialogue patterns.
        
        The standard sentence boundary detection fails for French dialogue
        where punctuation like ? and ! are followed immediately by lowercase
        letters (e.g., "demanda-t-il"). This method handles those cases.
        """
        if not text.strip():
            return text
        ...
        for para in paragraphs:
            ...
            for line in lines:
                ...
                # Check if this is a dialogue line starting with em-dash
                if re.match(r'^\s*\u2014\s+', line):
                    # Extract dialogue part after em-dash
                    dialogue_part = re.sub(r'^\s*\u2014\s+', '', line)
                    
                    # Split the dialogue on punctuation followed by space + lowercase
                    # or end of string (handles "demanda-t-il." pattern)
                    dialogue_parts = re.split(
                        r'(?<=[.!?;])\s+(?=[^A-Z\u00C0-\uDC00]|$)',
                        dialogue_part
                    )
                    ...
                    # Regular text - use standard sentence boundary
                    parts = re.split(
                        r'(?<=[.!?;])\s+(?=[^A-Z\u00C0-\uDC00]|$)',
                        line
                    )
    ...
    def _html_to_text(self, html_content: str) -> str:
        ...
        # Step 7: Split into sentences, handling French dialogue patterns
        text = self._split_into_sentences(text)
